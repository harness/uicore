...
```js
import { ButtonGroup, OptionsButtonGroup } from '@wings-software/uicore'
```
...
This component makes it easy to control buttons in a button group. It takes an array of ButtonProps where each item has two more extra optional fields value and selected, renders the ButtonGroup properly, and invokes onChange with the value of the selected item in the collection when the item is clicked.
