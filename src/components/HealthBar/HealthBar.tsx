...
export function HealthBar(props: HealthBarProps) {
  ...
  const healthBarColor = useMemo(() => (gradient ? { backgroundImage: gradient } : undefined), [gradient])
  return height > width ? (
    <Container data-name="verticalBar">
      <Container background={background} height={height} width={width} style={healthBarColor} />
