...
export function StepWizard<SharedObject = Record<string, unknown>>(
  ...
  const gotoStep = React.useCallback(
    (args: GotoStepArgs<SharedObject>) => {
      ...
      /* 
      *If stepNumber is greater than current totalSteps or equal to 0,
       do not set new state with updated stepNumber and return false.
       As its not a valid step stepNumber
      */
      if (stepNumber > state.totalSteps || stepNumber === 0) {
  ...
  React.useLayoutEffect(() => {
    if (Array.isArray(children)) {
      ...
      const getActiveStep = (prevState: StepState<SharedObject>) => {
        ...
        if (prevStepNames && prevChildren) {
          ...
          if (activeStepId.includes(STEP_ID_SEPARATOR)) {
            /*
             * Condition to handle case when nestedStepWizard is removed.
             * Make the parent step of the nestedStepWizard as the activeStep.
             */
            const currParentStepId = activeStepId.split(STEP_ID_SEPARATOR)[0]

            return (stepNames.findIndex(stepName => stepName.stepId === currParentStepId) ?? 0) + 1
          }
          return activeStep
        }
      ...
      setState(prevState => {
        // Update the activeStep when updating the children
