...
export function createLocaleStrings<T>(): CreateLocaleStringsReturn<T> {
    ...
    return {
      getString<K extends keyof T>(key: K, vars?: T[K]): string {
        ...
        if (typeof template !== 'string') {
          throw new Error(`No valid template with id "${key?.toString()}" found`)
        }
  ...
  function LocaleString<K extends keyof T>(props: LocaleStringProps<T, K>): React.ReactElement | null {
    ...
    try {
      ...
      return useRichText ? (
        <Tag {...(rest as Record<string, unknown>)} dangerouslySetInnerHTML={{ __html: text }} />
      ) : (
        <Tag {...(rest as Record<string, unknown>)}>{text}</Tag>
      )
    } catch (e: any) {
